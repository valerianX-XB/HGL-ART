...
export default function ControlPanel({state,setState,metrics,renderedCount,filteredCount,resetView,resetFilters,open,onClose,layersFocus}:any){
 ...
 return <aside className={`panel ${open?'open':'collapsed'}`} aria-label="Map Controls"><button className="panelClose" aria-label="Close controls panel" title="Close controls panel" onClick={onClose}>×</button><h2>{layersFocus ? 'Layers & Filters' : 'Map Controls'}</h2>
  {!layersFocus && <><label>Step 1: Analysis Layer<select value={state.analysisLayer} onChange={e=>update({analysisLayer:e.target.value})}>{Object.entries(LAYER_CONFIG).map(([k,v]:any)=><option key={k} value={k}>{v.label}</option>)}</select></label>
  ...
  {state.overlays.ooh && <details open={layersFocus || state.analysisLayer==='ooh'}><summary>OOH Filters</summary><h3>Types</h3><div className="filterGrid">{types.map(t=><label key={t}><input type="checkbox" checked={state.oohTypeFilters.has(t)} onChange={()=>toggleSet('oohTypeFilters',t)}/>{t}</label>)}</div><h3>Status</h3><div className="filterGrid">{statuses.map(t=><label key={t}><input type="checkbox" checked={state.oohStatusFilters.has(t)} onChange={()=>toggleSet('oohStatusFilters',t)}/>{t}</label>)}</div></details>}
  {state.overlays.anchors && <details open={layersFocus || true}><summary>Family Anchor Filters</summary><div className="filterGrid">{familyCategories.map(([t,hasRecords]:any)=><label key={t} className={!hasRecords?'disabled':''}><input type="checkbox" disabled={!hasRecords} checked={hasRecords && state.familyAnchorCategoryFilters.has(t)} onChange={()=>toggleSet('familyAnchorCategoryFilters',t)}/>{t}{!hasRecords?' — No records loaded':''}</label>)}</div></details>}
  {!layersFocus && <details className="advanced"><summary>Step 5: Advanced Visual Controls</summary>
