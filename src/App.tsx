...
import { createOohHeatmapLayer } from './layers/oohHeatmapLayer';
...
const familyAnchorAllCategories=[...familyAnchorCategories,'Birthday / camp / event'];

const initialState: MapState = validateMapState({
  analysisLayer: 'building',
  visualStyle: '3d',
  metricKey: 'hgl_building_opportunity_score',
...
export default function App(){
 const [zip,setZip]=useState<any>(),[centroids,setCentroids]=useState<any>(),[bg,setBg]=useState<any>(),[blocks,setBlocks]=useState<any>(),[buildings,setBuildings]=useState<any>(),[visualBuildings,setVisualBuildings]=useState<any>(),[streets,setStreets]=useState<any>(),[ooh,setOoh]=useState<any>(),[anchors,setAnchors]=useState<any>(),[schools,setSchools]=useState<any>(),[competitors,setCompetitors]=useState<any>(),[grid,setGrid]=useState<any>(),[metrics,setMetrics]=useState<MetricConfig[]>([]),[notes,setNotes]=useState<any>(),[summary,setSummary]=useState<any>();
 ...
 useEffect(()=>{Promise.all([
  loadJson('./data/hgl_zip_market_enriched.geojson'),loadJson('./data/hgl_zip_centroids.geojson'),loadJson('./data/block_groups_official_under5.geojson'),loadJson('./data/census_blocks_enriched.geojson'),loadJson('./data/buildings_residential_capacity_v5.geojson'),loadJson('./data/buildings_visual_all.geojson'),loadJson('./data/street_under5_capacity_signal.geojson'),loadJson('./data/ooh_assets_expanded_v4.geojson'),loadJson('./data/hgl_family_anchors.geojson'),loadJson('./data/schools_preschools.geojson'),loadJson('./data/competitors.geojson'),loadJson('./data/micro_market_grid.geojson'),loadJson('./data/metric_config.json'),loadJson('./data/source_notes_public.json'),loadJson('./data/hgl_zip_summary.json')]).then(([z,c,bg,bl,bu,vb,st,o,a,sc,co,g,m,n,s])=>{setZip(z);setCentroids(c);setBg(bg);setBlocks(bl);setBuildings(bu);setVisualBuildings(vb);setStreets(st);setOoh(o);setAnchors(a);setSchools(sc);setCompetitors(co);setGrid(g);setMetrics(m);setNotes(n);setSummary(s);});},[]);
 ...
 const layers=useMemo(()=>{
  ...
  if(layer==='building' && buildings) {
    if(style==='heatmap') arr.push(createBuildingHeatmapLayer(buildings,metric,true));
    else arr.push(createBuildingMassingLayer(buildings,metric,true,height.buildingHeightMultiplier,height.buildingHeightCapM,height.dataLiftMaxM,handleHover));
  }
  ...
  if(layer==='street' && streets) style==='heatmap'?arr.push(createOohHeatmapLayer(ooh,true)):arr.push(createStreetOpportunityRibbonLayer(streets,metric,height.streetSurfaceMaxM,true,handleHover));
 ...
 },[zip,centroids,bg,blocks,buildings,visualBuildings,streets,ooh,anchors,schools,competitors,grid,metric,layer,style,height,state.overlays,state.oohTypeFilters,state.oohStatusFilters,state.familyAnchorCategoryFilters,state.dataVersion,handleHover]);
 ...
 <ControlPanel state={state} setState={setState} metrics={metrics} renderedCount={layers.length} filteredCount={filteredOohCount} resetView={resetView} resetFilters={resetFilters}/>
 <section className="sideInfo"><Legend features={activeFeatures} metric={metric} metricConfig={selected}/><MetricCards features={centroids?.features||[]} summary={summary}/></section>
 <section className={showTable ? 'bottom open' : 'bottom collapsed'}><button onClick={()=>setShowTable(!showTable)}>{showTable ? 'Hide summary tables' : 'Show summary tables'}</button>{showTable && <><DataTable features={activeFeatures}/>{(state.overlays.ooh||layer==='ooh') && <OohAssetTable features={ooh?.features||[]}/>}</>}</section><ZipTooltip info={hover}/></div>
}
